...
- **Profile**: `Igniter::Workspace` + `Igniter::Application`, a packaged way to assemble an app and run it through a host adapter. The default host is `host :server`, and `require "igniter/cluster"` adds `host :cluster` for cluster-aware apps.
