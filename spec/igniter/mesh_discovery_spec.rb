...
RSpec.describe "Igniter Mesh — Phase 2: Dynamic Discovery" do
  ...
  # PeerRegistry
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::PeerRegistry do
    subject(:registry) { described_class.new }

    let(:peer_a) { Igniter::Mesh::Peer.new(name: "orders", url: "http://orders:4567", capabilities: [:orders]) }
    let(:peer_b) { Igniter::Mesh::Peer.new(name: "audit",  url: "http://audit:4567",  capabilities: [:audit]) }
    ...
    it "register is idempotent — latest version wins" do
      registry.register(peer_a)
      updated = Igniter::Mesh::Peer.new(name: "orders", url: "http://orders-v2:4567", capabilities: [:orders])
    ...
    it "is thread-safe under concurrent writes" do
      threads = 50.times.map do |i|
        Thread.new do
          p = Igniter::Mesh::Peer.new(name: "peer-#{i}", url: "http://p#{i}:4567")
          registry.register(p)
        end
      end
      threads.each(&:join)
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Config do
    ...
    it "has a PeerRegistry by default" do
      expect(config.peer_registry).to be_a(Igniter::Mesh::PeerRegistry)
    end
  ...
  # Announcer
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Announcer do
    let(:config) do
      Igniter::Mesh::Config.new.tap do |c|
  ...
  # Poller
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Poller do
    let(:config) do
      Igniter::Mesh::Config.new.tap do |c|
        c.peer_name          = "api-node"
  ...
  # Discovery
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Discovery do
    let(:config) do
      Igniter::Mesh::Config.new.tap do |c|
        c.peer_name = "api-node"
    ...
    let(:announcer_double) { instance_double(Igniter::Mesh::Announcer, announce_all: nil, deannounce_all: nil) }
    let(:poller_double)    { instance_double(Igniter::Mesh::Poller, poll_once: nil, start: nil, stop: nil, running?: false) }

    before do
      allow(Igniter::Mesh::Announcer).to receive(:new).and_return(announcer_double)
      allow(Igniter::Mesh::Poller).to receive(:new).and_return(poller_double)
    end
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  describe "Igniter::Mesh module" do
    it "start_discovery! starts discovery and returns self" do
      disc = instance_double(Igniter::Mesh::Discovery, start: nil, stop: nil, running?: true)
      allow(Igniter::Mesh::Discovery).to receive(:new).and_return(disc)

      result = Igniter::Mesh.start_discovery!

      expect(disc).to have_received(:start)
      expect(result).to be(Igniter::Mesh)
    end

    it "stop_discovery! stops discovery and clears the singleton" do
      disc = instance_double(Igniter::Mesh::Discovery, start: nil, stop: nil, running?: false)
      allow(Igniter::Mesh::Discovery).to receive(:new).and_return(disc)
      ...
      expect(Igniter::Mesh.instance_variable_get(:@discovery)).to be_nil
    end

    it "reset! stops discovery and clears config + router" do
      Igniter::Mesh.configure { |c| c.peer_name = "x" }
      disc = instance_double(Igniter::Mesh::Discovery, start: nil, stop: nil, running?: false)
      allow(Igniter::Mesh::Discovery).to receive(:new).and_return(disc)
      Igniter::Mesh.start_discovery!
      ...
      expect(disc).to have_received(:stop)
      expect(Igniter::Mesh.instance_variable_get(:@config)).to be_nil
      expect(Igniter::Mesh.instance_variable_get(:@router)).to be_nil
    end
  end
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Router, "merged peer routing" do
    let(:config) { Igniter::Mesh::Config.new }
    ...
    it "finds a dynamic peer when no static peers are configured" do
      config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "dyn-orders", url: "http://dyn:4567", capabilities: [:orders])
      )
    ...
    it "static peer takes precedence over same-named dynamic peer" do
      ...
      config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "orders-node", url: "http://dynamic:4567", capabilities: [:orders])
      )
    ...
    it "raises DeferredCapabilityError when all peers (static + dynamic) are dead" do
      ...
      config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "d", url: "http://d:4567", capabilities: [:orders])
      )
      stub_dead("http://s:4567")
      stub_dead("http://d:4567")

      expect { router.find_peer_for(:orders, deferred) }
        .to raise_error(Igniter::Mesh::DeferredCapabilityError)
    end

    it "resolve_pinned finds a dynamic peer by name" do
      config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "audit-node", url: "http://audit:4567", capabilities: [:audit])
      )
    ...
    it "resolve_pinned raises IncidentError when peer exists nowhere" do
      expect { router.resolve_pinned("ghost") }
        .to raise_error(Igniter::Mesh::IncidentError)
    end
  end
  ...
  describe Igniter::Server::Handlers::MeshPeersListHandler do
    ...
    it "returns empty array when Igniter::Mesh is not configured" do
    ...
    it "returns dynamic peers" do
      Igniter::Mesh.configure { |_c| }
      Igniter::Mesh.config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "dyn", url: "http://dyn:4567", capabilities: [:audit])
      )
    ...
    it "merges static + dynamic, static names win" do
      ...
      Igniter::Mesh.config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "shared", url: "http://dynamic:4567", capabilities: [:orders])
      )
      Igniter::Mesh.config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "dynamic-only", url: "http://d2:4567", capabilities: [:audit])
      )
  ...
  describe Igniter::Server::Handlers::MeshPeersRegisterHandler do
    ...
    before { Igniter::Mesh.configure { |_c| } }
    ...
    it "registers a peer and returns 200" do
      ...
      expect(Igniter::Mesh.config.peer_registry.peer_named("orders-node")).not_to be_nil
    end
    ...
    it "coerces capabilities to symbols in the registered peer" do
      ...
      peer = Igniter::Mesh.config.peer_registry.peer_named("x")
  ...
  describe Igniter::Server::Handlers::MeshPeersDeleteHandler do
    ...
    it "removes a registered peer and returns 200" do
      Igniter::Mesh.config.peer_registry.register(
        Igniter::Mesh::Peer.new(name: "orders-node", url: "http://orders:4567")
      )
      ...
      expect(Igniter::Mesh.config.peer_registry.peer_named("orders-node")).to be_nil
    end
