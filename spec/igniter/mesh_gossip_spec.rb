...
RSpec.describe "Igniter Mesh — Phase 3: Gossip Protocol" do
  ...
  def make_peer(name, url, caps = [])
    Igniter::Mesh::Peer.new(name: name, url: url, capabilities: Array(caps).map(&:to_sym))
  end
  ...
  describe "Config#gossip_fanout" do
    it "defaults to 3" do
      config = Igniter::Mesh::Config.new
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::GossipRound do
  ...
  describe "Convergence" do
    it "C discovers B by gossiping with A (A knows B, C only knows A)" do
      ...
      Igniter::Mesh::GossipRound.new(config).run
  ...
  describe "Poller#poll_once" do
    ...
    it "calls GossipRound#run when gossip_fanout > 0" do
      stub_seed_empty
      gossip = instance_double(Igniter::Mesh::GossipRound)
      allow(gossip).to receive(:run)
      allow(Igniter::Mesh::GossipRound).to receive(:new).with(config).and_return(gossip)

      Igniter::Mesh::Poller.new(config).poll_once
    ...
    it "skips GossipRound when gossip_fanout == 0" do
      ...
      expect(Igniter::Mesh::GossipRound).not_to receive(:new)
      Igniter::Mesh::Poller.new(config).poll_once
    end

    it "does not raise when registry is empty and fanout > 0" do
      stub_seed_empty
      expect { Igniter::Mesh::Poller.new(config).poll_once }.not_to raise_error
    end
  ...
  # ─────────────────────────────────────────────────────────────────────────────
  describe "Igniter::Mesh.start_discovery! with gossip_fanout = 0" do
    it "disables gossip without raising" do
      allow_any_instance_of(Igniter::Mesh::Announcer).to receive(:announce_all)
      ...
      expect { Igniter::Mesh.start_discovery! }.not_to raise_error
    ensure
