...
RSpec.describe Igniter::Replication::RoleRegistry do
  ...
  describe ".define" do
    it "returns a NodeRole" do
      ...
      expect(role).to be_a(Igniter::Replication::NodeRole)
    end
  ...
  describe ".fetch" do
    ...
    it "returns the role by symbol name" do
      expect(described_class.fetch(:worker)).to be_a(Igniter::Replication::NodeRole)
    end
