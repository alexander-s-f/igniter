...
RSpec.describe Igniter::Replication::SSHSession do
  let(:host)    { "10.0.0.1" }
  let(:user)    { "deploy" }
  ...
  describe "#exec!" do
    ...
    it "raises SSHError on non-zero exit" do
      ...
      expect { session.exec!("bad_cmd") }.to raise_error(
        Igniter::Replication::SSHSession::SSHError,
        /SSH command failed/
      )
    end
  ...
  describe "SSHError" do
    it "is a subclass of Igniter::Error" do
      expect(Igniter::Replication::SSHSession::SSHError.ancestors).to include(Igniter::Error)
    end
  end
end
