...
RSpec.describe "Igniter Mesh — Phase 1: Static Mesh" do
  ...
  # Errors
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::DeferredCapabilityError do
  ...
  describe Igniter::Mesh::IncidentError do
  ...
  describe Igniter::Mesh do
    ...
    it "router is a singleton" do
      expect(described_class.router).to be_a(Igniter::Mesh::Router)
  ...
  # Router
  # ─────────────────────────────────────────────────────────────────────────────
  describe Igniter::Mesh::Router do
    ...
    it "find_peer_for raises DeferredCapabilityError when no peers registered" do
      ...
      }.to raise_error(Igniter::Mesh::DeferredCapabilityError) { |e| expect(e.capability).to eq(:orders) }
    end
    ...
    it "resolve_pinned raises IncidentError for unknown peer" do
      ...
      }.to raise_error(Igniter::Mesh::IncidentError) { |e| expect(e.peer_name).to eq("audit-node") }
    end
  ...
  describe "Resolver integration" do
    ...
    context "with pinned_to: routing" do
      ...
      it "fails (node :failed) with IncidentError when pinned peer is down" do
        ...
        expect(audit_state.error).to be_a(Igniter::Mesh::IncidentError)
      end
