...
RSpec.describe "Igniter Mesh — GET /v1/mesh/sd (Prometheus HTTP SD)" do
  ...
  describe "response shape" do
    before do
      ...
      Igniter::Mesh.config.peer_registry.register(make_peer("node-a", "http://node-a:4567", %i[orders inventory]))
    end
    ...
    it "capabilities label is empty string when peer has no capabilities" do
      Igniter::Mesh.reset!
      Igniter::Mesh.configure { |c| c.discovery_interval = 60 }
      Igniter::Mesh.config.peer_registry.register(make_peer("bare", "http://bare:4567"))
  ...
  describe "host:port extraction" do
    ...
    [
      ...
      it "extracts #{expected} from #{url}" do
        Igniter::Mesh.config.peer_registry.register(make_peer("p", url))
        entry = parsed_body(call_handler).first
    ...
    it "falls back to the raw URL if URI parsing fails" do
      bad_url = "not a url !!!"
      Igniter::Mesh.config.peer_registry.register(make_peer("broken", bad_url))
      entry = parsed_body(call_handler).first
  ...
  describe "static + dynamic peer merge" do
    it "includes both static and dynamic peers" do
      ...
      Igniter::Mesh.config.peer_registry.register(make_peer("dynamic-peer", "http://dynamic:4567", [:orders]))
    ...
    it "static peer takes precedence when name collides with dynamic" do
      ...
      Igniter::Mesh.config.peer_registry.register(make_peer("shared", "http://dynamic-shared:4567", [:orders]))
