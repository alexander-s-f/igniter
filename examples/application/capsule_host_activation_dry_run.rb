...
Dir.mktmpdir("igniter-capsule-host-dry-run") do |root|
  ...
  puts "application_capsule_host_activation_dry_run=#{dry_run.fetch(:dry_run)}"
  puts "application_capsule_host_activation_committed=#{dry_run.fetch(:committed)}"
  puts "application_capsule_host_activation_executable=#{dry_run.fetch(:executable)}"
  puts "application_capsule_host_activation_would_apply=#{dry_run.fetch(:would_apply).length}"
  puts "application_capsule_host_activation_skipped=#{dry_run.fetch(:skipped).length}"
  puts "application_capsule_host_activation_refusals=#{dry_run.fetch(:refusals).length}"
  puts "application_capsule_host_activation_warnings=#{dry_run.fetch(:warnings).length}"
  puts "application_capsule_host_activation_surfaces=#{dry_run.fetch(:surface_count)}"
end
