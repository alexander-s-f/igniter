...
Dir.mktmpdir("igniter-capsule-host-plan-verify") do |root|
  ...
  verification = Igniter::Application.verify_host_activation_plan(activation_plan).to_h

  puts "application_capsule_host_activation_plan_verify_valid=#{verification.fetch(:valid)}"
  puts "application_capsule_host_activation_plan_verify_executable=#{verification.fetch(:executable)}"
  puts "application_capsule_host_activation_plan_verify_operations=#{verification.fetch(:operation_count)}"
  puts "application_capsule_host_activation_plan_verify_verified=#{verification.fetch(:verified).length}"
  puts "application_capsule_host_activation_plan_verify_findings=#{verification.fetch(:findings).length}"
  puts "application_capsule_host_activation_plan_verify_surfaces=#{verification.fetch(:surface_count)}"
end
