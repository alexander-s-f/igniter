...
module Companion
  # ResearchSkill — a sub-agent that researches a topic in depth.
  #
  # From the parent ChatExecutor's perspective this looks exactly like a Tool:
  # it has a schema, a name, and is called with a single :topic parameter.
  # Internally it runs its own LLM loop (when COMPANION_REAL_LLM=1) using
  # sub-tools to search and synthesize information.
  #
  ...
  class ResearchSkill < Igniter::AI::Skill
    ...
    tools SaveNoteTool, GetNotesTool  # can save findings to notes during research
    ...
    def call(topic:)
      if ENV["COMPANION_REAL_LLM"]
        # Real path: runs internal tool loop for multi-source synthesis
        complete(
