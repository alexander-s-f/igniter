...
module Companion
  module Dashboard
    module AssistantFeedbackHandler
      module_function

      FEEDBACK_ACTIONS = {
        "useful" => :feedback_useful,
        "too_verbose" => :feedback_too_verbose,
        "too_slow" => :feedback_too_slow,
        "wrong_lane" => :feedback_wrong_lane
      }.freeze
      ...
      def call(params:, body:, headers:, env:, raw_body:, config:) # rubocop:disable Lint/UnusedMethodArgument
        ...
        feedback = body.fetch("feedback", "").to_s
        action = FEEDBACK_ACTIONS[feedback]
        base_path = base_path_for(env)

        if request_id.empty? || action.nil?
          return render_error(env: env, base_path: base_path, error_message: "Request and valid feedback are required.")
        end
...

        {
          status: 303,
          body: "",
          headers: { "Location" => Handlers::Support.route_for(base_path, "/assistant") + "?assistant_feedback=1" }
        }
