...
module Companion
  module Dashboard
    module AssistantRedeliverHandler
      module_function
      ...
      def call(params:, body:, headers:, env:, raw_body:, config:) # rubocop:disable Lint/UnusedMethodArgument
        ...
        Companion::DashboardApp.interface(:assistant_api).redeliver_request(request_id: request_id)

        {
          status: 303,
          body: "",
          headers: { "Location" => Handlers::Support.route_for(base_path, "/assistant") + "?assistant_redelivered=1" }
        }
