            ...
            <body>
              <main class="shell">
                ...
                <section class="grid">
                  ...
                  <article class="panel">
                    <h2>Telegram</h2>
                    #{telegram_markup(snapshot.fetch(:telegram))}
                  </article>
                  <article class="panel">
                    <h2>Notes</h2>
      ...
      def reminders_markup(reminders)
        ...
        items = reminders.last(8).reverse.map do |reminder|
          "<li><strong>#{h(reminder["task"])}</strong> <span class=\"pill\">#{h(reminder["timing"])}</span>" \
            "<br><code>channel=#{h(reminder["channel"] || "none")} chat=#{h(reminder["chat_id"] || "-")}</code></li>"
        end
        "<ul>#{items.join}</ul>"
      end

      def telegram_markup(telegram)
        ...
        if bindings.empty?
          ...
          items = bindings.first(6).map do |binding|
            ...
            "<li><strong>#{h(name)}</strong><br><code>chat_id=#{h(binding["chat_id"])} type=#{h(binding["type"] || "unknown")}</code></li>"
          end
