...
- orchestration inbox items now also carry canonical `action_history`, so operator APIs and pages can show not only current state but the audit trail of `opened`, `acknowledged`, `handoff`, `resolved`, and `dismissed` transitions with source and operation metadata
