...
- `Igniter::App::Observability::OperatorActionHandler` now packages operator-side mutations as a sibling custom-route handler, so mounted surfaces can `wake`, `handoff`, `approve`, `reply`, `complete`, or `dismiss` orchestration items through the same app DSL surface
...
- `mount_operator_surface` now gives a built-in operator console page above that same API, including execution drill-down, filter-driven inspection, and row-level operator actions without adding a frontend dependency
- orchestration inbox items now also carry canonical `action_history`, so operator APIs and pages can show not only current state but the audit trail of `opened`, `acknowledged`, `handoff`, `resolved`, and `dismissed` transitions with explicit operator identity fields like `actor`, `origin`, and `actor_channel`
- that same operator query surface can now also filter, facet, and order by `latest_action_actor`, `latest_action_origin`, and `latest_action_source`, so audit trails are queryable as a first-class operator dimension instead of only embedded history
