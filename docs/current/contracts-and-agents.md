...
- `igniter-app` now also ships `Igniter::App::Observability::OperatorActionHandler`, so the mounted operator plane can mutate orchestration state through the same mounted surface instead of inventing a second workflow API
