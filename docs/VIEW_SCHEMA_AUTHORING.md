# View Schema Authoring

This guide captures the current public authoring story for `Igniter::Plugins::View`
schemas.

Use it when you want to define a page as persisted JSON-like data and render it
through `Igniter::Plugins::View::SchemaRenderer`.
...
- `training-checkin`
  contract-backed workflow form
  uses `notice`, `fieldset`, `actions`
- `weekly-review`
  lightweight persisted form
  uses `notice`, multiple `fieldset`s, `actions`

When the companion dashboard is seeded, both examples are available through:

- `/views/training-checkin`
- `/views/weekly-review`

The companion dashboard home page also exposes a lightweight catalog/editor
surface for these schemas:
...
- shared rendering through the Tailwind view plugin
