...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Companion | Personal assistant shell for ordinary users. | Credentials setup -> reminders/trackers/countdowns -> daily summary. | `igniter-application`, `igniter-contracts`, `igniter-ai`, `igniter-web`. | Setup state and deterministic daily summary. | SQLite app store; live AI remains explicit opt-in. | `ruby examples/application/companion_poc.rb` | `ruby examples/application/companion_poc.rb server` |
