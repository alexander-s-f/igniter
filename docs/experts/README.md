...
| ★ [igniter-lang/igniter-lang-temporal.md](igniter-lang/igniter-lang-temporal.md) · [ru](igniter-lang/igniter-lang-temporal.ru.md) | **FRONTIER** — Temporal dimension — `History[T]`, `[t]` operator, `T ⊑ History[T]`, rule declarations with algebra + conflict detection, orthogonality principle (add rule = zero existing contract changes), temporal invariants (frozen/consistency/monotone), counterfactual reports, situation calculus foundation (PTIME), 5 open directions: bitemporal, temporal synthesis, causal chains, probabilistic rules, distributed time |
