...
| [igniter-science-critical.md](igniter-science-critical.md) · [ru](igniter-science-critical.ru.md) | **Science / Robotics / Space / Medicine validation** — per-domain pain analysis, direct primitive mapping, DSL sketches (protein fold pipeline, autonomous navigation + swarm, spacecraft thermal control + mission rules, medication dosing + BiHistory audit); cross-domain synthesis table; **6 platform insights**: physical unit types, invariant severity levels, deadline contracts, uplink-able rule declarations, certified graph export, uncertainty propagation; verdict: confirmed with differentiation |
