...
| ★ [igniter-lang/igniter-lang-temporal-deep.md](igniter-lang/igniter-lang-temporal-deep.md) · [ru](igniter-lang/igniter-lang-temporal-deep.ru.md) | **FRONTIER** — Full temporal model: bitemporal `BiHistory[T]` (`T⊑History⊑BiHistory`, 4 canonical queries), goal-directed rule synthesis (LP→PTIME), causal chain detection (RDG, 3 resolution strategies), probabilistic rules (`~applies`, two-level evaluation), distributed time (Lamport clocks, causal `as_of`), unified 3-axis contract parameters, 3-iteration POC roadmap (~750 LOC) |
