...
- `C2-I` is intentionally appended after the closed authorization/status
  receipt. It executes the exact next implementation route authorized by
  `C1-A` and curated by `C2-S`.
...
status: closed
closed_by: S3-R147-C2-S / igniter-lang/docs/tracks/stage3-round147-status-curation-v0.md
...
authorized_write_scope: igniter-lang/lib/igniter_lang/fragment_registry_compatibility_adapter.rb; igniter-lang/experiments/fragment_registry_compatibility_adapter_helper_implementation_proof/**; igniter-lang/docs/tracks/fragment-registry-compatibility-adapter-helper-implementation-proof-v0.md
...
- Implement only:
  ...
  - the exact R146 helper input/result shape;
  - the exact R146 selection rules:
    - if `oof` present, selected `oof`;
    - elsif `temporal` present, selected `temporal`;
    - elsif `escape` present, selected `escape`;
    - elsif `stream` present, selected `escape`;
    - elsif `epistemic` present, selected `epistemic`;
    - else selected `core`.
- Preserve:
  - direct-require-only helper;
  - no root require from `igniter-lang/lib/igniter_lang.rb`;
  - no classifier wiring;
  - no live classifier dispatch;
  - helper result internal-only.
- Prove:
...
- Dynamic closed-surface checks must verify:
  - helper file exists at the authorized path;
  - root require is absent;
  - classifier does not reference the helper;
  ...
  - no `CompilationReport`, `CompilerResult`, loader/report, or
    CompatibilityReport output changed;
  - no PROP-036 or PROP-038 mutation occurred.
- Broad negative vocabulary scan:
  - scan every `igniter-lang/lib/igniter_lang/*.rb`;
  - forbidden outside the authorized helper file:
...
- Required commands:
  ...
  - TypeChecker proof if applicable to touched paths
  - `ruby igniter-lang/experiments/invariant_severity_proof/invariant_severity_proof.rb`
- Record pinned check counts:
  - classifier: 21 named checks;
  - contract modifiers: 20 named checks;
  - assumptions: 39 named checks;
  - source-to-SemanticIR: 31 named checks;
  - igapp assembler: 17 named checks;
  - invariant severity: 34 named checks;
  - helper implementation proof: pinned by this card.
- Record byte-for-byte parity evidence for classifier, contract-modifier,
  assumptions, SemanticIR, and `.igapp` artifacts. If any proof command only
  regenerates outputs, the implementation proof harness must compare paths and
  digests itself.
- Do not edit outside the authorized write scope.
- Do not root-require the helper.
- Do not wire classifier or replace `contract_fragment_for`.
- Do not edit parser, classifier, TypeChecker, SemanticIR, assembler, report,
  `.igapp`, public API/CLI, runtime, Spark, production, proposal, or existing
  golden files.

Deliver:
- `igniter-lang/lib/igniter_lang/fragment_registry_compatibility_adapter.rb`
- Proof runner and summary JSON under
  `igniter-lang/experiments/fragment_registry_compatibility_adapter_helper_implementation_proof/`
- Track doc in `igniter-lang/docs/tracks/`
- Command matrix with pinned counts
