module Lab.Fixtures.FieldSupplyWatch
...
olap_point regional_supply: Integer {
  dims {
    time: Timestamp
    region: String
    sku: String
...
contract NormalizeReport(report: FieldReport, as_of: Timestamp) -> signal: DemandSignal using audited_mesh {
  compute evidence = EvidenceBundle {
    primary: [report.source],
    corroborating: [],
    contradictions: []
  }

  compute signal = DemandSignal {
...
contract VerifyDemand(signal: DemandSignal, as_of: Timestamp) -> verified: DemandSignal, risk: RiskScore using audited_mesh {
  ...
  compute corroboration = count_matching_reports(recent_reports, signal.clinic, signal.item)
  compute contradiction = count_contradictions(recent_reports, signal.clinic, signal.item)

  compute risk = RiskScore {
...
contract BuildRegionalPosture(region: Region, item: SupplyItem, as_of: Timestamp, knowledge_as_of: Timestamp) -> posture: RegionalSupplyPosture using audited_mesh {
  ...
  compute demand_units = sum_requested_units(demand_events, region, item)
  compute available_units = sum_offer_units(offers, region, item) + available_stock(stock, region, item)
  compute shortage_units = max(0, demand_units - available_units)
  ...
  compute posture = RegionalSupplyPosture {
    region: region,
    item: item,
...
contract PlanDispatch(posture: RegionalSupplyPosture, as_of: Timestamp) -> plan: ShipmentPlan, receipt: DispatchDecisionReceipt using audited_mesh {
  ...
  compute candidate_suppliers = rank_suppliers(offers, posture.region, posture.item, posture.shortage_units)

  mesh SupplyAnalysisMesh route_plan {
    input posture
    input candidate_suppliers
    admit where trust_tier >= :regional_operator
    retry max 3
    timeout 30.seconds
  } -> route_options

  compute plan = choose_plan(route_options, posture.shortage_units)

  human_review dispatch_override when posture.risk.value >= 0.700 {
    subject plan
    reason "high risk dispatch"
    required_role :regional_operator
  } -> override: Option[HumanOverride]

  compute receipt = DispatchDecisionReceipt {
    decision_id: content_hash(plan, posture, as_of),
    plan: plan,
    risk: posture.risk,
    caused_by: obs_refs([posture, offers, route_options]),
...
contract MonitorSupply(region: Region, item: SupplyItem, as_of: Timestamp, knowledge_as_of: Timestamp) -> posture: RegionalSupplyPosture, dispatch: Option[ShipmentPlan], receipt: Option[DispatchDecisionReceipt] using audited_mesh {
  fold_stream report_ingress window rolling 6.hours into report_batch {
    step acc, report -> acc.add(NormalizeReport(report, as_of).signal)
  }

  compute verified_signals = map(report_batch, signal -> VerifyDemand(signal, as_of).verified)

  compute posture = BuildRegionalPosture(region, item, as_of, knowledge_as_of).posture

  compute dispatch_needed = posture.shortage_units > 0 || posture.risk.value >= 0.700

  compute decision = if dispatch_needed {
