  ...
  class Application
    ...
    class ClusterAppHostConfig
      ...
      attr_accessor :peer_name, :local_capabilities, :seeds, :discovery_interval,
                    :auto_announce, :local_url, :gossip_fanout, :start_discovery
      ...
      def add_peer(name, url:, capabilities: [])
        @peers << Peer.new(name: name.to_s, url: url, capabilities: Array(capabilities).map(&:to_sym))
        self
      end
      ...
      def to_h
        {
          ...
          peers: peers.map { |peer| { name: peer.name, url: peer.url, capabilities: peer.capabilities } }
        }
      end
    end
  end
