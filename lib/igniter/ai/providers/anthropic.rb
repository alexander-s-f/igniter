    ...
    module Providers
      ...
      class Anthropic < Base # rubocop:disable Metrics/ClassLength
        ...
        def post(path, body) # rubocop:disable Metrics/MethodLength,Metrics/AbcSize
          ...
          raise Igniter::AI::ProviderError, "Cannot connect to Anthropic API: #{e.message}"
        end
        ...
        def handle_response(response) # rubocop:disable Metrics/MethodLength
          unless response.is_a?(Net::HTTPSuccess)
            ...
            raise Igniter::AI::ProviderError, "Anthropic API error #{response.code}: #{error_msg}"
          end

          JSON.parse(response.body)
        rescue JSON::ParserError => e
          raise Igniter::AI::ProviderError, "Anthropic returned invalid JSON: #{e.message}"
        end
