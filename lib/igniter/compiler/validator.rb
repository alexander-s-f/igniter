    ...
    class Validator
      ...
      def validate_class_callable_signature!(node, callable)
        if callable <= Igniter::Executor
          ...
          raise validation_error(node, "Compute '#{node.name}' class callable must respond to `.call`")
        end
      end

      def validate_object_callable_signature!(node, callable)
        unless callable.respond_to?(:call)
          raise validation_error(node, "Compute '#{node.name}' callable object must respond to `call`")
        end
