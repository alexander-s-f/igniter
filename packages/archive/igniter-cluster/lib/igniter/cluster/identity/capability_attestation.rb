    ...
    module Identity
      class CapabilityAttestation
        attr_reader :peer_name, :node_id, :algorithm, :public_key, :url,
                    :capabilities, :tags, :metadata, :observed_at, :signature

        def self.build(identity:, peer_name:, url:, capabilities:, tags:, metadata:, observed_at: Time.now.utc.iso8601)
          payload = {
        ...
        def initialize(hash)
          ...
          @observed_at = source[:observed_at].to_s.freeze
