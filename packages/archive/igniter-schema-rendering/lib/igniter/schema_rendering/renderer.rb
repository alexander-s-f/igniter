...
module Igniter
  module SchemaRendering
    class Renderer < Igniter::Frontend::Component
      ...
      def call(view)
        view.raw(
          Igniter::Frontend::Tailwind.render_page(
            ...
            main.component(Igniter::Frontend::Tailwind::UI::SubmissionNotice.new(message: notice, tone: :notice, tag: :div)) if notice
      ...
      def render_node(view, node) # rubocop:disable Metrics/CyclomaticComplexity,Metrics/MethodLength
        ...
        when "notice"
          view.component(Igniter::Frontend::Tailwind::UI::SubmissionNotice.new(message: node.fetch("message"), tone: notice_tone(node["tone"]), tag: :div))
        when "heading"
      ...
      def render_form_child(form, target, node, inline_actions:)
        ...
        when "fieldset"
          ...
          when "notice"
            target.component(
              Igniter::Frontend::Tailwind::UI::SubmissionNotice.new(message: node.fetch("message"), tone: notice_tone(node["tone"]), tag: :div)
            )
          when "actions"
            target.component(
              Igniter::Frontend::Tailwind::UI::InlineActions.new do |actions|
                Array(node["children"]).each do |child|
        ...
        when "input"
            target.component(
              Igniter::Frontend::Tailwind::UI::FieldGroup.new(id: dom_id(node), label: node.fetch("label"), error: field_error) do |field|
                Igniter::Frontend::FormBuilder.new(field).input(
                  node.fetch("name"),
                id: dom_id(node),
        ...
        when "textarea"
            target.component(
              Igniter::Frontend::Tailwind::UI::FieldGroup.new(id: dom_id(node), label: node.fetch("label"), error: field_error) do |field|
                Igniter::Frontend::FormBuilder.new(field).textarea(
                  node.fetch("name"),
                id: dom_id(node),
                value: value_for(node),
                placeholder: node["placeholder"],
        ...
        when "select"
            target.component(
              Igniter::Frontend::Tailwind::UI::ChoiceField.new(
                kind: :select,
        ...
        when "checkbox"
          ...
          when "submit"
            if inline_actions
              Igniter::Frontend::FormBuilder.new(target).submit(node.fetch("label"), class: submit_classes)
            else
              target.component(
                Igniter::Frontend::Tailwind::UI::InlineActions.new do |actions|
                  Igniter::Frontend::FormBuilder.new(actions).submit(node.fetch("label"), class: submit_classes)
                end
              )
          end
        when "text"
      ...
      def field_classes
        ...
        def submit_classes
          Igniter::Frontend::Tailwind::UI::Tokens.action(variant: :primary, theme: :orange)
        end
      ...
      def notice_tone(value)
        ...
        def ui_theme
          Igniter::Frontend::Tailwind::UI::Theme.fetch(:schema)
        end
