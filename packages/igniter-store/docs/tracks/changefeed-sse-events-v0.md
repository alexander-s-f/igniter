...
- Add `GET /v1/events` to the HTTP surface used by `StoreServer`.
...
Prefer a small handler/adapter over embedding logic deep inside StoreServer:

```text
HTTPAdapter::EventsHandler or StoreServer SSE helper
  -> parse cursor/stores
...
- StoreServer currently owns a `ChangefeedBuffer`; make sure `/v1/events` is
...
Track: igniter-store/changefeed-sse-events-v0
