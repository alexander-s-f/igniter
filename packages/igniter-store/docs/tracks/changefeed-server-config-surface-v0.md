...
Expose Changefeed production knobs through the StoreServer configuration
surface.
...
thresholds. But `StoreServer` still constructs:
...
StoreServer currently does not expose these knobs through:

- `ServerConfig`
- `StoreServer.new(...)`
- `exe/igniter-store-server`
- docs/examples
...
StoreServer convention.

## CLI Surface

Extend `exe/igniter-store-server` with explicit Changefeed options.
...
`StoreServer#observability_snapshot[:changefeed]` should reflect the configured
...
- `StoreServer.new(config: config)` builds `ChangefeedBuffer` with config values.
- Direct `StoreServer.new(changefeed: {...})` overrides config values.
...
Track: igniter-store/changefeed-server-config-surface-v0
